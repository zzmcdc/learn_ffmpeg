...
__global__ static void YuvToRgbPlanarKernel(uint8_t *pYuv, int nYuvPitch,
  ...
  // bgr data bgr|bgr|bgr
  uint8_t *pDst = pRgbp + x * 3 + y * nRgbpPitch * 3;

  // b need fill 4 position
  *pDst = rgb0.v.x;
  *(pDst + 3) = rgb1.v.x;
  // next line
  *(pDst + nRgbpPitch * 3) = rgb2.v.x;
  *(pDst + nRgbpPitch * 3 + 3) = rgb3.v.x;

  pDst += 1;

  *pDst = rgb0.v.y;
  *(pDst + 3) = rgb1.v.y;
  *(pDst + nRgbpPitch * 3) = rgb2.v.y;
  *(pDst + nRgbpPitch * 3 + 3) = rgb3.v.y;

  pDst += 1;
  *pDst = rgb0.v.z;
  *(pDst + 3) = rgb1.v.z;
  *(pDst + nRgbpPitch * 3) = rgb2.v.z;
  *(pDst + nRgbpPitch * 3 + 3) = rgb3.v.z;
}
...
void YUV444ToColor32(uint8_t *dpYUV444, int nPitch, uint8_t *dpBgra,
  ...
  Yuv444ToRgbKernel<
      uchar2, COLOR32,
      uint2><<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2), dim3(32, 2)>>>(
...
void YUV444ToColorPlanar(uint8_t *dpYUV444, int nPitch, uint8_t *dpBgrp,
                         int nBgrpPitch, int nWidth, int nHeight, int iMatrix) {
  SetMatYuv2Rgb(iMatrix);
  Yuv444ToRgbPlanarKernel<
      uchar2, COLOR32,
      uchar2><<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2), dim3(32, 2)>>>(
