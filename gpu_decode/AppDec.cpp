  ...
  for (int iGpu = 0; iGpu < nGpu; iGpu++) {
    ...
    for (int i = 0; i < sizeof(aeCodec) / sizeof(aeCodec[0]); i++) {
      ...
      std::cout << "Codec"
                ...
                << "  " << decodeCaps.nMaxMBCount << '\t' << "MinWidth"
                << "  " << decodeCaps.nMinWidth << '\t' << "MinHeight"
                << "  " << decodeCaps.nMinHeight << std::endl;
    }
...
/**
*  This sample application illustrates the demuxing and decoding of media file
* with
*  resize and crop of the output image. The application supports both planar
* (YUV420P and YUV420P16)
